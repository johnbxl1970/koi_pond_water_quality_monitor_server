...
- `POST /api/admin/device-claims` and `GET /api/admin/stats` are guarded by `AdminGuard` (shared bearer token via `ADMIN_API_TOKEN`). Separate from user auth by design — the flashing tool and the admin dashboard are not users. The admin Next.js app at `admin/` reads `ADMIN_API_TOKEN` from its own `.env.local` (`KOI_ADMIN_TOKEN`) for server-side fetches; the token never reaches the browser. Phase 1 will add a `User.isAdmin` flag + login screen to replace this with per-user admin auth.
