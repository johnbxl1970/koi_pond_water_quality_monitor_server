...
- **`PredictionEvent` + `ModelVersion`** are written by the sidecar directly via psycopg (`ml/db_writes.py`). Every successful `/retrain/anomaly` mints a `ModelVersion` (per-pond scope via `pondId`, prior active row in scope deactivated atomically) and every served `/predict/anomaly` writes a `PredictionEvent` linked to the active model. The A/B promotion gate (only flip `isActive` after candidate beats incumbent on held-out data) lands in Phase 1; today every fresh fit auto-activates.
- **Nightly retrain.** `src/predictions/predictions.scheduler.ts` runs `@Cron(EVERY_DAY_AT_3AM)` and POSTs to the sidecar's `/retrain/anomaly`. Lives in NestJS rather than the sidecar so cron infra stays consolidated with the existing weather-poll cron. Failures are logged and silently retried the next night — never throw out of the scheduler.
...
- **Two admin auth flavors, by design.**
  - `POST /api/admin/device-claims` is guarded by the legacy `AdminGuard` (shared `ADMIN_API_TOKEN` bearer). The flashing tool isn't a user — keep it on the shared bearer.
  - All human-facing admin routes (`GET /api/admin/stats`, `GET /api/admin/{users,ponds,devices,alerts,predictions}`) use `AdminUserGuard` — valid JWT bearer + `User.isAdmin = true`. Promotion is intentional and manual via `npx ts-node scripts/promote-admin.ts <email>`; the regular signup flow never grants admin.
- The admin Next.js app at `admin/` does cookie-based session auth: `loginAction` calls `/api/auth/login`, verifies `isAdmin` via `/api/auth/me`, then sets two HttpOnly cookies (`koi_admin_at`, `koi_admin_rt`). Edge middleware at `admin/src/middleware.ts` decodes the access token's `exp` and transparently refreshes via `/api/auth/refresh` when the access JWT is within 30s of expiring — keeps admins signed in across the 15-min access TTL without forcing relogins.
